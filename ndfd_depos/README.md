...
Generates a neutrino event (genie w/ ND flux) and runs particle propagation (edep-sim) in large liquid argon volume (LArBath). Event goes through translation and rotation (around beam axis) throws in ND to find a realisation that is selected. This event is moved to FD (Earth's curvature correction applied) and translation throws applied until a selected realisation is found. The result is the truth level information required to produce ND-FD pairs.

## System

Recommend to do this on `dunegpvmXX` so that `jobsub` can be used for grid jobs

## Setup

```
# Get code on ndfd_pairs branch
...
`--disk` and `--expected_lifetime`. The file argument to the script (`0` in above) is just the
...
* `fd_deps` (selected FD event charge depositions)
```
...
* `primaries` (4-vectors of interaction primary particle)
```
dtype([('eventID', '<u4'), ('pdg', '<u4'), ('p0_MeV', '<f4'), ('p1_MeV', '<f4'), ('p2_MeV', '<f4'), ('p3_MeV', '<f4')])
```

* `segments` (selected ND event charge depositions)
```
